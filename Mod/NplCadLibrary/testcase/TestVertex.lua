...
function TestVertex.test_init()
	...
	assert(v1.pos:equals(pos) and v1.normal:equals(normal));
end
function TestVertex.test_clone()
	...
	local v1 = CSGVertex:new():init(pos,normal);
	local v2 = v1:clone();
	assert(v1.pos:equals(v2.pos) and v1.normal:equals(v2.normal));
end
function TestVertex.test_clone()
	local pos =  TestFrame.randomVector3d();
	local normal = TestFrame.randomNormal3d();
...
function TestVertex.test_flip()
	...
	local v1 = CSGVertex:new():init(pos,normal);
	v1:flip();
	assert(v1.pos:equals(pos) and v1.normal:equals(normal:negated()));
end
function TestVertex.test_interpolate()
	...
	assert(v1.pos:equals(pos1) and v1.normal:equals(normal1));
end	