...
function CSGLine2D:equals(l)
    return (l.normal.equals(self.normal) && (l.w == self.w));
end
...
function CSGLine2D:xAtY(y)
    ...
    local x = (self.w - self.normal._y * y) / self.normal.x;
    return x;
end
...
function CSGLine2D:intersectWithLine(line2d)
    local point = CSG.solve2Linear(self.normal.x, self.normal.y, line2d.normal.x, line2d.normal.y, self.w, line2d.w);
